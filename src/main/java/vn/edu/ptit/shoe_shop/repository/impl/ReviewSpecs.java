...
public class ReviewSpecs {
    ...
    public static Specification<Review> productIdEquals(UUID productId) {
        return (root, query, cb) -> {
            ...
            Join<?,?> variant = root.join("variant");
    ...
    public static Specification<Review> variantIdEquals(UUID variantId) {
        return (root, query, cb) -> variantId == null ? cb.conjunction() : cb.equal(root.get("variant").get("productVariantId"), variantId);
    }

    public static Specification<Review> sizeEquals(String size) {
        return (root, query, cb) -> {
